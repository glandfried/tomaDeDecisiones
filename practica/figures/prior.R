...
sorpresa_de_ganar <- function(s_1,mu,sigma,beta=25/7.5){
...
ejes <- function(){
  ...
  mtext(text= expression(Habilidad[a]),side =1,line=2,cex=2.5)
  mtext(text ="Densidad" ,side =2,line=1,cex=2.5)
  
}

x.min <- min(mu1_grilla)
x.max <- max(mu1_grilla)
posterior2 <- prior(mu1_grilla,mu,sigma)/max(prior(mu1_grilla,mu,sigma))
...
white=0.9
polygon(xx,yy,col=rgb(0,0,0,0.15),border=F)

which.max(posterior2)

yy2 <- c(posterior2[seq(230,330)],rep(0,length(mu1_grilla[seq(230,330)])))
xx2 <- c(mu1_grilla[seq(230,330)],rev(mu1_grilla[seq(230,330)]))      

polygon(xx2,yy2,col=rgb(0,0,0,0.35),border=F)
...
lines(mu1_grilla,posterior2 ,lty=1,lwd=1)
