...
axis(lwd=1,side=1, at=tb,labels=expression( "("~s[1]~"-"~mu[2]~")" ),las=0,cex.axis=1.5,line=-0.3)

mtext(text ="Densidad" ,side =2 ,line=1.5,cex=2)
mtext(text ="Diferencia de rendimientos",side =1 ,line=2.3,cex=2)
abline(v=tb,lty=3)
...
polygon(xx,yy,col=rgb(0,0,0,0.33))
...
mid <- length(D)%/%2
h <- dnormal[mid]/4
text(D[mid]+110,h, "Gana 1", col = 1, cex=2.1)
